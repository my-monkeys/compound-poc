export default function SimCard({ sim, active, onSelect, onDelete }) {
  ...
  return (
    ...
    >
      <div style={{ width: 6, height: 6, borderRadius: '50%', flexShrink: 0, background: active ? '#e2cb95' : '#4a5568' }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ color: active ? '#e8eaf0' : '#c9d1d9', fontSize: 12, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {name}
        </div>
        <div style={{ color: active ? '#4a5568' : '#3a4455', fontSize: 10, marginTop: 1 }}>
          {params.years} ans
        </div>
      </div>
      {active && (
        <button
          ...
          style={{ background: 'none', border: 'none', color: '#2e3545', fontSize: 14, lineHeight: 1, padding: '0 2px' }}
